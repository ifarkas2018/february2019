...
public List<EmpNameInfo1p1> getName( String empID ) {
 	...
 	// SQL_EMP_NAME - the String that contains the query onto which one the WHERE clause will be added depending on the value of the empID 
...
public List<EmpNameInfo1p1> getEmployeeID( String firstName, String lastName) {
	...
	// SQL_EMP_ID - the String that contains the query onto which one the WHERE clause will be added depending on the value of the firstName, lastName  
	String sql = EmpNameMapper1p1.SQL_EMP_ID;
	try {
	   	Object[] params = new Object[] {};
	   	// EmpNameMapper is a mapping class that maps 1 column in the query statement to 1 field in the model class ( EmpNameInfo.java )
	   	EmpNameMapper1p1 mapper = new EmpNameMapper1p1();
	   	// running the query and retrieving the employee ID for the employee with the certain name
	   	list = this.getJdbcTemplate().query(sql, params, mapper);
	} catch (Exception e) {
...
public void addToQueryStrID(String firstName, String lastName ) {
	...
	boolean firstNameEx; // does the first name exists
	boolean lastNameEx; // does the last name exists
